...
df = \
    (df.set_index(df.columns.drop("INSTRUCTOR IN CHARGE/Instructor", 1).tolist())["INSTRUCTOR IN CHARGE/Instructor"].str.split(',', expand=True)
     .stack()
     .reset_index()
...
for i in range(len(new[1])):
    if(new[1][i] != None and pd.isnull(new[1][i]) == False):
        pos_list.append(i)

for x in pos_list:
    df["DAYS/ H"][x] = str(df["DAYS/ H"][x])+"?"
...
df = \
    (df.set_index(df.columns.drop("DAYS/ H", 1).tolist())["DAYS/ H"].str.split(',', expand=True)
     .stack()
     .reset_index()
